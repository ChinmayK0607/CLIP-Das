...
import clip  # official OpenAI CLIP (ensure installed from GitHub)
...
# -------------------------------
# Serve index.html by reading the file.
# -------------------------------
...
for model_str, data_str in models_to_load:
    ...
    if data_str is not None:  # OpenCLIP models
        ...
        # Using official CLIP load function.
...
def run_das(request: DASRequest):
    ...
    results = generate_image(
        ...
        target_texts_and_values=[(request.weight, request.prompt)],
        starting_image=None,
        steps=request.steps,
        augmentation_copies=8,
        jitter_scale=28,
